...
export default function BimListView(props: any) {
        ...
        views.length === 0 ? <div className="bimNoViews">Hi, you have no saved views.</div>: 
            <div className="bimList">
                {views.map((item: any, index: number) => {
                    return (
                        <div key={item.id} className={'listItem' +  ((item.id === props.selectedItem) ? ' active' : '')} >
                            <div className="title"  onClick={() => props.setSelected(item.id)}>{viewNameMapping[item.viewName] || item.viewName}</div>
                            <div className="operation">
                                {(item.id === props.selectedItem) ?<VisibilityIcon fontSize={'small'} /> : null }
                                {
