...
} from '../../../projects/Context/ProjectDetailsActions';
...
export default function UpdateProject(props: any): ReactElement {
    ...
    if (Number(pathMatch.params.projectId)) {
      try {
        ...
        const ProjectToken: any = {
          ...
          features: ['FORMS', 'PROJECT_SETUP', 'MASTER_PLAN','BIM', 'DRAWINGS', "PLAN_COMPONENT","SPECIFICATIONS","DMS", "DAILYLOG"],
        };
  ...
  useEffect(() => {
    if (!projectDetailsState?.projectUpdatePermission?.canUpdateMyProject) {
      projectDetailsDispatch(setDisableUpdatePermission(true));
    } else {
      projectDetailsDispatch(setDisableUpdatePermission(false));
    }
  }, [
    canUpdateProject,
  ...
  return (
    <div className={`updateProjects`}>
      <div className="fullSideBar">
        <div
          className="updateProjects__info"
        ...
        <div
          className="updateProjects__details"
          style={{ flexBasis: `${projectTab === 'DETAILS' ? '50%' : '75%'}` }}
        >
          <div className="updateProjects__details__detailsContainer">
            <ProjectDetails />
          </div>
        </div>
        {confirmOpen ? (
