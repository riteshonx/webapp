...
function CustomTextInput({
  ...
  const { setIsDirty, setCurrentSubjectValue, setValue, setError }: any =
    useContext(formStateContext);
  useEffect(() => {
    if (formData?.elementId) {
      setIsDisabled(
        submitalsFields.indexOf(formData?.elementId) > -1 && isSpecification
      );
    }
  }, [formData]);
    ...
    <Controller
      render={({ field }: { field: any }) => (
        <TextField
          ...
          autoComplete="off"
          onChange={(e) => {
            field.onChange(e), setIsDirty(true), changeInValue(e);
          }}
