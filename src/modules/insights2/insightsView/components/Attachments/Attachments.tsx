...
function Attachments({ form }: { form: Form }): ReactElement {
  ...
  return (
    <div className="v2-lessons-learned-attachments">
      <h5>Attachments</h5>
      {loading ? <div className="v2-lessons-learned-attachments__loader"></div> :
          <div className="v2-lessons-learned-attachments__container">
            {urls.map(url => {
              return (
                <div className="v2-lessons-learned-attachments__cover">
                  <img src={url} ></img>
                  <div className="v2-lessons-learned-attachments__mask">
                    <CloudDownload
                      onClick={() => downloadAttachment(url)}
                      className="v2-lessons-learned-attachments__mask__icon"
                    />
                  </div>
                </div>
              )
