...
import { priorityPermissions } from 'src/modules/dynamicScheduling/permission/scheduling';
...
import './EditTaskDetailsViewRelatedTask.scss';
const EditTaskDetailsViewRelatedTask = () => {
  ...
  const { getRelatedTasks, relatedTasks, clearRelatedTasks } =
  ...
  useEffect(() => {
    getChildTasks(currentTask.id);
    getRelatedTasks(currentTask.id);
  ...
  const getChildTasks = (id: any) => {
    const tasks: any = [];
    const children = gantt.getChildren(id);
    children.forEach(async (id: any, index: any) => {
      const task = gantt.getTask(id);
      let constraintVarianceValue = 0;
      if (task.status === 'To-Do') {
        constraintVarianceValue = await getConstraintsByTaskId(task.id);
        tasks.push({
          ...task,
  ...
  const getPrecedingTasksConstraintVariance = (relatedTasks: any) => {
    ...
    relatedTasks.forEach(async (element: any, index: any) => {
      ...
      constraintVarianceValue.constraint = await getConstraintsByTaskId(
  ...
  const getActionMenuHeight = () => {
    const length = [
      childTask.length,
      precedingTask.length,
      succeedingTask.length,
    ].filter((length) => length > 0).length;

    return length == 3
      ? 'edit-task-details-view-related-task__action-menu__height-141'
      : length == 2
      ? 'edit-task-details-view-related-task__action-menu__height-100'
      : 'edit-task-details-view-related-task__action-menu__height-47';
  };
  return (
    <div style={{ display: 'flex' }}>
      {(childTask.length > 0 ||
        ...
        <div
          className={`edit-task-details-view-related-task__action-menu ${getActionMenuHeight()}`}
        >
          {childTask.length > 0 && (
            <a
              href="#childTask"
            ...
            >
              <img
                src={
                  activeTab === 'childTask' ? AccountTreeActive : AccountTree
                }
                alt="task-tree"
                className={`edit-task-details-view-related-task__action-menu__task-tree   `}
              ></img>
            </a>
          )}
          {precedingTask.length > 0 && (
            ...
            >
              ...
              <img
                ...
                alt="preceding-task"
                className={`edit-task-details-view-related-task__action-menu__preceding-task ${
                  succeedingTask.length === 0
                    ? 'edit-task-details-view-related-task__action-menu__mb-13'
                    : ''
                }`}
              ></img>
            </a>
          ...
          {succeedingTask.length > 0 && (
            ...
            >
              <img
                ...
                alt="succeeding-task"
                className="edit-task-details-view-related-task__action-menu__succeeding-task"
              ></img>
            </a>
          )}
        </div>
      ...
      <div className="edit-task-details-view-related-task">
        {childTask.length === 0 &&
          precedingTask.length === 0 &&
          succeedingTask.length === 0 && (
            <div className="edit-task-details-view-related-task__empty">
              <img
                src={EmptyRelatedTask}
                alt="no related task"
                className="edit-task-details-view-related-task__empty-img"
              />
              <span className="edit-task-details-view-related-task__empty-text-1">
                Looks like your task does not have any buddies yet!
              </span>

              <span className="edit-task-details-view-related-task__empty-text-2">
        ...
        {childTask.length > 0 && (
          <span
            className="edit-task-details-view-related-task-heading"
            id="childTask"
          >
            Child Tasks
          </span>
        ...
        {childTask.length > 0 && (
          <div className="edit-task-details-view-related-task__data">
            <TableContainer>
              <Table aria-labelledby="tableTitle" area-label="enhanced table">
                <TableHead>
                  <TableRow>
                    <TableCell className="edit-task-details-view-related-task__data__table-head-cell">
                      Name
                    </TableCell>
                    <TableCell className="edit-task-details-view-related-task__data__table-head-cell">
                      Status
                ...
                <TableBody className="edit-task-details-view-related-task__data__table-body">
                      ...
                      <TableRow
                        ...
                        className={`${
                          task.status === 'Complete'
                            ? 'edit-task-details-view-related-task__data__table-body__table-row-complete'
                            : ''
                        }`}
                      >
                        <TableCell className="edit-task-details-view-related-task__data__table-body__table-row__table-cell-name">
                          {task.text}
                        </TableCell>
                        <TableCell className="edit-task-details-view-related-task__data__table-body__table-row__table-cell-status">
                          {task.status === 'Complete' ? (
                            <span className="edit-task-details-view-related-task__data__table-body__table-row__table-cell-status-complete">
                              Completed
                            </span>
                          ) : (
                            task.status
                        ...
                        <TableCell className="edit-task-details-view-related-task__data__table-body__table-row__table-cell-blockers">
                          <div className="edit-task-details-view-related-task__data__table-body__table-row__table-cell__align-center">
                            {task.status === 'To-Do' ? (
                              <img
                                className="edit-task-details-view-related-task__data__table-body__icon"
                                src={ConstraintsIcon}
                                alt="constraint"
                              ></img>
...
export default EditTaskDetailsViewRelatedTask;
