...
import {
  ...
  GET_ATTCHED_FILES,
  SAVE_ATTCHED_FILES,
} from 'src/modules/dynamicScheduling/graphql/queries/attachment';
import {
  permissionKeysByAssigneeAndToken,
  priorityPermissionsByToken,
} from 'src/modules/dynamicScheduling/permission/scheduling';
...
import './CommonFileAttachment.scss';
...
function CommonFileAttachment(props: any): ReactElement {
  ...
  useEffect(() => {
    getAttchedFiles(currentTask);
  }, []);

  const getAttchedFiles = async (currentTask: any) => {
    try {
  ...
  const deleteAttachedFile = async (id: any, currentTask: any) => {
    try {
      const res = await client.mutate({
  ...
  const fileUpload = async (acceptedfilesData: any) => {
    ...
    acceptedfilesData.forEach((file: any) => {
      fileData = {
        fileName: file?.name,
        projectId: Number(currentTask.projectId),
        featureId:
          priorityPermissionsByToken(
            'create',
            projectTokens[currentTask.projectId]
  ...
  const removeFile = (file: any, index: number) => {
    if (!props.isEditAllowed) {
      ...
      deleteAttachedFile(attachedFileData[index].id, currentTask);
      if (newUploadedfile && newUploadedfile.length > 0) {
  ...
  const renderAcceptedFiles = attachedFile?.map((file: any, index: number) => (
    <>
      <div
        ...
        className="common-file-attachment__viewList__file"
      >
        <div className="common-file-attachment__viewList__file__thumbnail">
          {/* <img src={file.preview} /> */}
          <DescriptionIcon />
        </div>
        <div className="common-file-attachment__viewList__file__description">
          <Tooltip title={file.fileName}>
            <div className="common-file-attachment__viewList__file__description__fileName">
              {file.fileName.length > 20
                ? file.fileName.slice(0, 18)
                : file.fileName}
            </div>
          </Tooltip>
          <div className="common-file-attachment__viewList__file__description__fileSize">
            {(file.fileSize / (1024 * 1024)).toFixed(3)} MB
          </div>
        </div>
        {true && (
          <div className="common-file-attachment__viewList__file__download">
        ...
        {progressValue > 1 && progressValue < 99
          ? ''
          : !props.isEditAllowed && (
              <div className="common-file-attachment__viewList__file__fileRemove">
  ...
  const viewrenderAcceptedFiles = attachedFile?.map(
  ...
  return (
    <section className="common-file-attachment">
      {permissionKeysByAssigneeAndToken(
          ...
          {({ getRootProps, getInputProps }) => (
            <>
              <div>
                <div
                  {...getRootProps({
                    ...
                    className:
                      !props?.attachment?.bgColor && attachedFile?.length == 0
                        ? 'dropzone, common-file-attachment__dropZone--white'
                        : 'dropzone, common-file-attachment__dropZone',
                  })}
                >
                  <input {...getInputProps()} />
                  {props?.attachment?.placeholder ? (
                    <p className="common-file-attachment__text">
                      {props?.attachment?.placeholder}
                    </p>
                  ) : (
                    <p className="common-file-attachment__text">
                      Drag & Drop files to attach or,{' '}
                      <b className="common-file-attachment__text__click">
                        click
                      </b>{' '}
                      here to add files
                    </p>
              ...
              {attachedFile?.length > 0 && (
                <div className="common-file-attachment__viewList">
                  <div
                    style={{
                      maxWidth: '100%',
                      display: 'grid',
...
export default React.memo(CommonFileAttachment);
