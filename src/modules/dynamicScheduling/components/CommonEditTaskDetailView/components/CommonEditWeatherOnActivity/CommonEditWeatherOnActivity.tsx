...
import './CommonEditWeatherOnActivity.scss';
...
function CommonEditWeatherOnActivity(props: any): ReactElement {
  const didMount = React.useRef(false);
  const { state, dispatch }: any = useContext(stateContext);
  const { currentTask, projectTokens } = props;

  const [weatherTemplates, setWeatherTemplate] = useStateCallback(
    {} as IWeatherTemplateMap
  );
  const [initialRender, setInitialRender] = useStateCallback(true);
  const [projectMetrics, setProjectMetrics] = useStateCallback(
    {} as IProjectMetrics
  );
  const [weatherConstraint, setWeatherConstraint] = useStateCallback(
    defaultParameterValues
  );
  const [weatherConstraintObj, setWeatherConstraintObj] = useStateCallback(
    {} as IWeatherConstraint
  );
  ...
  const fetchProjctMetrics = async () => {
    ...
    const projectMetricsStatic = fetchProjectMetrics.data.project[0]
      ?.metrics || {
      temprature: '',
      unitOfMeasurement: '',
    };
    localStorage.setItem(
      'projectMetrics',
      JSON.stringify(projectMetricsStatic)
    );
  ...
  const fetchWeatherConstraintTemplate = async () => {
    ...
    weatherTemplate.data.weatherConstraintTemplate.forEach(
      (ele: IWeatherTemplate) => {
        templateMap[ele.id] = {
          ...ele,
  ...
  const changeTemplate = (value: string) => {
    ...
    if (!(value in weatherTemplates)) {
      setWeatherConstraint(cloneDeep(defaultParameterValues), () => {
  ...
  const siToLocalUnitConversion = (parameter: any) => {
    ...
    if (projectMetricsStatic.unitOfMeasurement === 'Metrics') {
      tempParameter.wind =
        parameter.wind === '' ? '' : parameter.wind * 1.609344;
      tempParameter.windGust =
        parameter.windGust === '' ? '' : parameter.windGust * 1.609344;
    }
  ...
  return (
    <div className="common-weather-activity">
      <div className="common-weather-activity__template">
        <label htmlFor="weather_template_type">Select Template</label>
        <div className="common-weather-activity__template__group">
          <select
            id="weather_template_type"
          ...
          >
            ...
            {Object.values(weatherTemplates).map(
              (weatherTemplate: IWeatherTemplate) => {
                return (
                  <option key={weatherTemplate.id} value={weatherTemplate.id}>
          ...
          <HtmlTooltip
            ...
            title={
              <React.Fragment>
                Please select a template to apply values. You will be able to
                override each weather parameter once a template is selected.
              </React.Fragment>
            }
          >
            <InfoOutlined className="common-weather-activity__info" />
          </HtmlTooltip>
        </div>
      </div>
      <div className="common-weather-activity__parameter">
        <div className="common-weather-activity__parameter__label">
          Rain/Snow
        </div>
        <div className="common-weather-activity__parameter__value">
          <input
            type="checkbox"
            disabled={disabledInput}
            checked={weatherConstraint.rain}
            onChange={() => {
              changeValue(!weatherConstraint.rain, 'rain');
            }}
          />
          <span className="common-weather-activity__parameter__value__label">
            Impacted?
          </span>
        </div>
      </div>
      <div className="common-weather-activity__parameter">
        <div className="common-weather-activity__parameter__label">Wind</div>
        <div className="common-weather-activity__parameter__value">
          <input
            type="number"
          ...
          />
          <span className="common-weather-activity__parameter__value__unit">
      ...
      <div className="common-weather-activity__parameter">
        ...
        <div className="common-weather-activity__parameter__value temperature__value">
          <span className="common-weather-activity__parameter__value__range">
            Min
          </span>
          <input
            type="number"
      ...
      <p className="common-weather-activity__msg">
...
export default CommonEditWeatherOnActivity;
