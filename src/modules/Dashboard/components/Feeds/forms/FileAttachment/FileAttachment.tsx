...
export const FileAttachment = (props: any): React.ReactElement => {
	...
	const renderAttachment =
		...
		attachmentList.map((attachmentFile: any, index: number) => {
			const fileSize = attachmentFile?.attachment?  attachmentSize(attachmentFile?.attachment?.fileSize) :attachmentSize(attachmentFile?.fileSize)
			return (
				<>
					<div
						key={
							`${attachmentFile?.attachment?.blobkey}-${index}` ||
							`${attachmentFile?.blobkey}-${index}`
						}
					...
					>
								...
								<div className="attachment-fileList-file-description-fileName">
									{attachmentFile?.attachment?.fileName
										? attachmentFile.attachment.fileName.length > 20
											? `${attachmentFile.attachment.fileName.slice(
													0,
													18
											  )}....${
											...
											: attachmentFile.attachment.fileName
										: attachmentFile?.fileName.length > 20
										? `${attachmentFile.fileName.slice(0, 18)}....${
												attachmentFile.fileName.split('.')[1]
										  }`
										: attachmentFile.fileName}
								</div>
							</Tooltip>
						...
						<div className="attachment-fileList-file-download">
							<GetAppIcon
								onClick={() => {
									handleDownloadAttachment(attachmentFile?.attachment || attachmentFile);
								}}
