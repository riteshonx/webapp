...
import "../DashboardSlate2/Components/ConnectorsScreen/ConnectorsScreen.scss";

export default function AgaveLink({ status }: any): React.ReactElement {
  const {
    dispatch,
    state: { sourceSystem },
  ...
  return (
    ...
    >
      {status ? `Connect` : `3rd party login`}
    </Button>
  );
}
