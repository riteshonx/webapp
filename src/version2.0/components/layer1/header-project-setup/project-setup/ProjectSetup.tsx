...
import { genFetchProjectNames } from '../../../../../services/projectServices';
...
function ProjectSetup(): ReactElement {
	...
	const getActiveTenant = async () => {
		try {
			...
			if (projectNamesResponse?.data?.project?.length) {
				...
				let targetTenant;
				for (let start = 0; start < allTokens.length; start++) { // what is this i
	...
	const checkOtherTenants = async (projectId: number, menuType: string) => {
		...
		if (decodeToken().tenants.length > Object.entries(allTokens).length) {
			const tenants = Object.entries(allTokens).map((allToken: any) => Number(allToken[0])); // use token as param

			const userTenantIds = decodeToken().tenants.filter(
				(eToken: any) => {
			...
			for (let i = 0; i < userTenantIds.length; i++) {
				...
				promiseList.push(
					postApi('V1/user/login/exchange', exchangeToken)
				);
			}
	...
	useEffect(() => {
		if (
			...
			state.currentProject?.projectName !== 'All'
		){
			...
			<div className="header__left__portfolio">
				<>
					...
					<DropdownSearch
						...
						isDisabled={
							state.currentLevel !== 'tenant' ? false : true
						}
						type={'portfolio'}
						transparent={
