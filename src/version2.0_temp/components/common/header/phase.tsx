...
export const Phase = (props: any): React.ReactElement => {
  ...
  const getPhaseList = (phaseList: Array<PhaseItem>) => {
    return phaseList.map((item: PhaseItem) => {
      return (
        <div className="v2-header-phase-item">
          <div
            className="s-v-center s-pointer"
            onClick={() => toggleCheckedPhaseItem(item)}
          >
            <div
              className={`v2-header-phase-item-checkbox ${
                checkedPhaseItem.includes(item.id) ? 'selected' : ''
              }`}
            ></div>
  ...
  return (
    <div className="v2-header-phase">
      <div className="v2-header-phase-header">Phases</div>
      ...
      <div className="v2-header-phase-action s-v-center">
        ...
        <button onClick={onCancel} className="v2-header-phase-action-cancel">
          Cancel
        </button>
      </div>
    </div>
